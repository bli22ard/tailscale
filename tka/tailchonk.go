...
type Chonk interface {
	// GetAUM returns the AUM with the specified digest.
	//
	// If the AUM does not exist, then os.ErrNotExist is returned.
	GetAUM(hash AUMHash) (AUM, error)

	// GetChildAUMs returns all AUMs with a specified previous
	// AUM hash.
	GetChildAUMs(prevAUMHash AUMHash) ([]AUM, error)
	...
	// GetHeads returns AUMs for which there are no children. In other
...
// GetAUM returns the AUM with the specified digest.
func (c *Mem) GetAUM(hash AUMHash) (AUM, error) {
	c.l.RLock()
	defer c.l.RUnlock()
	aum, ok := c.aums[hash]
	if !ok {
...
// GetOrphans returns all AUMs which do not have a parent.
func (c *Mem) GetOrphans() ([]AUM, error) {
	c.l.RLock()
	defer c.l.RUnlock()
	out := make([]AUM, 0, 6)
	for _, a := range c.aums {
...
// GetChildAUMs returns all AUMs with a specified previous
// AUM hash.
func (c *Mem) GetChildAUMs(prevAUMHash AUMHash) ([]AUM, error) {
	c.l.RLock()
	defer c.l.RUnlock()
	out := make([]AUM, 0, 6)
