...
func TestTailchonkMem_GetChildAUMs(t *testing.T) {
	chonk := Mem{}
	...
	stored, err := chonk.GetChildAUMs(parentHash)
	if err != nil {
		t.Fatalf("GetChildAUMs failed: %v", err)
	}
...
func TestTailchonkMem_GetOrphans(t *testing.T) {
	chonk := Mem{}
...
func TestTailchonkMem_ReadChainFromHead(t *testing.T) {
	...
	gotIntermediate, err := chonk.GetAUM(parent)
	if err != nil {
		t.Fatalf("GetAUM(<intermediate>) failed: %v", err)
	}
	...
	gotGenesis, err := chonk.GetAUM(parent)
	if err != nil {
		t.Fatalf("GetAUM(<genesis>) failed: %v", err)
	}
